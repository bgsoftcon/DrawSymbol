...
OpenLayers.Editor = OpenLayers.Class({
    ...
    */
    ...
    editorControls: ['CleanFeature', 'DeleteFeature', 'DeleteAllFeatures','Dialog', 'DrawHole','DrawRegular',
        'DrawPolygon', 'DrawPath', 'DrawPoint', 'EditorPanel','ImportFeature','DrawSymbol',
    ...
    initialize: function (map, options) {
        ...
        if (options.styleMap) {
			...
			//Fuellfarben nach mouseclick
                'default': new OpenLayers.Style({
				//6to3dgitHEX:http://maettig.com/?page=PHP/CSS_Color_Converter
				//Farbtabelle :http://www.yellowpipe.com/yis/tools/hex-to-rgb/color-converter.php
                    //fillColor: '#07f',
					fillColor: '#28F',
                    fillOpacity: 0.7,
					//fillOpacity: 0.8,
...
//					labelYOffset: 0,
				...
				//Digitalisier und Selektier Farben aus Menueleiste
        ...
        for (var i = 0, il = this.featureTypes.length; i < il; i++) {
            if (this.featureTypes[i] == 'polygon') {
                ...
                this.activeControls.push('DrawSymbol');
            }
    ...
    addEditorControls: function () {
        ...
        for (var i = 0, len = editor.activeControls.length; i < len; i++) {
            ...
            switch (control) {
				...
				case 'DrawText':
                    controls.push(new OpenLayers.Editor.Control.DrawText(editor.editLayer,
                            OpenLayers.Util.extend({}, this.DrawText)
                    ));
                    break;
